...
from nova_python_app.backbone.store.models import ProgramInstance
...
async def get_programs(
    program_service: ProgramAPIServiceInterface = Depends(Provide[NovaContainer.services.program_service])
):
...
async def create_program(
    ...
    if request.template_name not in REGISTERED_PROGRAM_TEMPLATES:
        raise HTTPException(status_code=404, detail=f"Program template '{request.template_name}' not found")
    
    # Check if program already exists
    existing_instance = program_service.get_program(program_name)
    if existing_instance:
        raise HTTPException(status_code=409, detail=f"Program instance '{program_name}' already exists")
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail=f"Failed to save program instance '{program_name}' to database")
        
        return {"message": f"Program instance '{program_name}' created successfully", "template_name": request.template_name, "data": request.data}
        ...
        raise HTTPException(status_code=400, detail=f"Invalid data for template '{request.template_name}': {error_details}")
...
async def update_program(
    ...
    if template_name not in REGISTERED_PROGRAM_TEMPLATES:
        raise HTTPException(status_code=500, detail=f"Template '{template_name}' not found in registered templates")
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail=f"Failed to update program instance '{program_name}' in database")
        
        return {"message": f"Program instance '{program_name}' updated successfully", "data": data}
    
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Invalid data for program instance '{program_name}': {str(e)}")
...
async def delete_program(
    ...
    if not success:
        raise HTTPException(status_code=500, detail=f"Failed to delete program instance '{program_name}' from database")
...
async def get_database_stats(
    db_connection: DatabaseConnectionInterface = Depends(Provide[NovaContainer.stores.database_connection])
):
