...
async def execute_program_async(
    ...
    try:
        ...
        print(
            f"Program execution completed with processor: {result.get('processor_type', 'unknown')}"
        )
        ...
        if run_service:
            try:
                ...
                run_service.update_run_status(
                    run_id, "failed", finished_at=finished_at, error_message=str(e), exit_code=1
                )
...
async def create_program_run(
    ...
    run_service: ProgramRunAPIServiceInterface = Depends(
        Provide[NovaContainer.services.program_run_service]
    ),
    ...
    if not created_run_id:
        raise HTTPException(status_code=500, detail="Failed to save program run to database")
...
async def get_program_run_status(
    ...
    if not run_data:
        raise HTTPException(status_code=404, detail=f"Program run '{run_id}' not found")

    # Verify the run belongs to the specified program
    if run_data["program_name"] != program_name:
        raise HTTPException(
            status_code=404, detail=f"Program run '{run_id}' not found for program '{program_name}'"
        )
...
async def list_program_runs(
    ...
    return ProgramRunList(
        runs=[ProgramRunStatus(**run) for run in paginated_runs], total_count=total_count
    )
...
async def delete_program_run(
    ...
    if run_data["status"] == "running":
        raise HTTPException(
            status_code=400, detail="Cannot delete a running program. Stop it first."
        )

    # Use injected run store
    success = run_store.delete(run_id)
    if not success:
        raise HTTPException(status_code=500, detail="Failed to delete program run")
