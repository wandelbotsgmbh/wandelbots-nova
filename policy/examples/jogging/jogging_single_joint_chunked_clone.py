"""Single-arm joint jogging with 8-step chunks on the virtual ur10e-clone.

Mirrors jogging_single_joint_chunked.py but targets the virtual ur10e-clone and
the robot's current home pose, so it can be dry-run before moving the real ur10e.

Run:
    NOVA_API=http://wandelbox-hhmnwy PYTHONPATH=. python \
        policy/examples/jogging/jogging_single_joint_chunked_clone.py
"""
...
@nova.program(
    id="jogging_single_joint_chunked_clone",
    name="Single-Arm Joint Jogging Chunked (clone)",
