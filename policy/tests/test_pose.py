...
def test_rotation_vector_passthrough():
    result = pose_to_tcp(_pose(1000, 2000, 3000, 0.1, 0.2, 0.3), "rotation_vector")
    assert len(result) == 6
...
def test_quaternion_identity():
    ...
    result = pose_to_tcp(_pose(0, 0, 0, 0, 0, 0), "quaternion")
    assert len(result) == 7
...
def test_quaternion_90deg_z():
    ...
    result = pose_to_tcp(_pose(0, 0, 0, 0, 0, angle), "quaternion")
...
def test_quaternion_unit_norm():
    ...
    result = pose_to_tcp(_pose(100, 200, 300, 0.7, -0.3, 1.2), "quaternion")
...
def test_rot6d_identity():
    ...
    result = pose_to_tcp(_pose(0, 0, 0, 0, 0, 0), "rot6d")
    assert len(result) == 9
...
def test_rot6d_90deg_z():
    ...
    angle = math.pi / 2
    result = pose_to_tcp(_pose(0, 0, 0, 0, 0, angle), "rot6d")
...
def test_rot6d_orthogonality():
    ...
    result = pose_to_tcp(_pose(500, -200, 800, 1.0, -0.5, 0.8), "rot6d")
    col1 = result[3:6]
    col2 = result[6:9]
    # Unit length
