...
class EventRecorder:
    ...
    def record_stage_duration(
        self, robot_id: str, stage_name: str, duration_seconds: float
    ) -> None:
    ...
    async def _handle_request(self, message: RequestZone) -> None:
        ...
        if owner is None:
            ...
            await message.reply_to.send(
                ZoneGranted(zone=message.zone, stage_name=message.stage_name)
            )
            return
...
class RobotActor(Actor):
    ...
    async def handle(self, message: Any) -> None:
        ...
        if isinstance(message, AbortRun):
            self._aborting = True
            self._recorder.record(
                actor=self.name, event="abort_run", robot_id=self.robot_id, detail=message.reason
            )
            if self._waiting_for_zone is not None and not self._waiting_for_zone.done():
    ...
    async def _bootstrap(self) -> None:
        ...
        try:
            ...
            self._recorder.record(
                actor=self.name, event="bootstrap_ready", robot_id=self.robot_id, detail=self._tcp
            )
            ...
            await self._orchestrator.send(
                StageFailed(robot_id=self.robot_id, stage_name="bootstrap", reason=str(exc))
            )
        finally:
        ...
        try:
            if step.zone is not None:
                ...
                await self._plc.send(
                    RequestZone(
                        zone=step.zone, robot_id=self.robot_id, stage_name=stage.name, reply_to=self
                    )
                )
            ...
            if self._active_zone is not None:
                ...
                await self._plc.send(
                    ReleaseZone(zone=zone_name, robot_id=self.robot_id, stage_name=zone_stage)
                )
                self._recorder.record(
                    actor=self.name,
                    event="zone_released",
            ...
            if not self._aborting:
                await self._orchestrator.send(
                    StageFailed(robot_id=self.robot_id, stage_name=stage.name, reason=str(exc))
                )
        finally:
...
class OrchestratorActor(Actor):
    ...
    async def _handle_stage_completed(self, message: StageCompleted) -> None:
        self._recorder.record_stage_duration(
            message.robot_id, message.stage_name, message.duration_seconds
        )
        self._recorder.record(
            actor=self.name,
...
def _compose_offset_pose(
    home_pose: Pose, offset: tuple[float, float, float, float, float, float]
) -> Pose:
...
async def build_approach_plan(context: RobotBehaviorContext) -> RobotPlan:
    ...
    return RobotPlan(
        actions=[joint_ptp(context.home_joints), cartesian_ptp(target_pose)], tcp=context.tcp
    )
...
@nova.program(
    ...
    preconditions=nova.ProgramPreconditions(
        controllers=[
            virtual_controller(
                name=spec.controller_name, manufacturer=spec.manufacturer, type=spec.controller_type
            )
