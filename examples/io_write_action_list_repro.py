"""
Minimal repro for `io_write` behavior inside action lists.

This example:
- creates the required boolean bus IOs through the API before running motions
- executes several short out-and-back `ptp` sequences with `io_write` actions
  placed at different positions in the action list
- includes mixed `async_action` + `io_write` cases
- includes pure `async_action` cases with no motions
- includes one case with only writes and no motions

Run:
    uv run python examples/io_write_action_list_repro.py
"""

import asyncio

import wandelbots_api_client.v2_pydantic as wb

import nova
from nova import api, run_program
from nova.actions import (
    ActionExecutionContext,
    async_action,
    await_action,
    get_default_registry,
    ptp,
    register_async_action,
)
from nova.actions.io import io_write
from nova.cell.controllers import virtual_controller
from nova.program import ProgramPreconditions
from nova.types import MotionSettings
from nova.types.pose import Pose
from nova.utils.io import get_bus_io_value, set_bus_io_value

SIGNALS = [
    "io_write_case_start",
    "io_write_case_middle",
    "io_write_case_end",
    "io_write_case_many",
    "io_write_case_write_only_a",
    "io_write_case_write_only_b",
    "io_write_case_async_a",
    "io_write_case_async_b",
]


async def setup_signals(ctx: nova.ProgramContext) -> None:
    for signal in SIGNALS:
        try:
            await ctx.nova.api.bus_ios_api.add_profinet_io(
                cell=ctx.cell.id,
                io=signal,
                profinet_io_data=wb.ProfinetIOData(
                    description=signal,
                    type=wb.ProfinetIOTypeEnum.PROFINET_IO_TYPE_BOOL,
                    direction=wb.ProfinetIODirection.PROFINET_IO_DIRECTION_OUTPUT,
                    byte_address=800,
                    bit_address=None,
                ),
            )
        except Exception as exc:
            print(f"Signal '{signal}' may already exist: {exc}")


async def reset_signals() -> None:
    await set_bus_io_value({signal: False for signal in SIGNALS})


async def short_async_marker(ctx: ActionExecutionContext) -> None:
    label = ctx.action_kwargs.get("label", "async")
    print(f"[async] start {label} at {ctx.trigger_location:.3f}")
    await asyncio.sleep(0.1)
    print(f"[async] end {label}")


def register_example_async_actions() -> None:
    action_name = "examples.io_write_action_list_repro.short_async_marker"
    registry = get_default_registry()
    if not registry.is_registered(action_name):
        register_async_action(action_name, short_async_marker)


def log_case(name: str, values: dict[str, bool | int | float]) -> None:
    print(f"{name}: {values}")


@nova.program(
    name="IO Write Action List Repro",
    preconditions=ProgramPreconditions(
        controllers=[
            virtual_controller(
                name="ur10e",
                manufacturer=api.models.Manufacturer.UNIVERSALROBOTS,
                type=api.models.VirtualControllerTypes.UNIVERSALROBOTS_UR10E,
            )
        ],
        cleanup_controllers=False,
    ),
)
async def main(ctx: nova.ProgramContext) -> None:
    register_example_async_actions()
    await setup_signals(ctx)
    await reset_signals()

    controller = await ctx.cell.controller("ur10e")
    motion_group = controller[0]
    tcp = (await motion_group.tcp_names())[0]

    start_pose = await motion_group.tcp_pose(tcp)
    forward_pose = start_pose @ Pose((40, 0, 0, 0, 0, 0))
    fast_back_and_forth = MotionSettings(tcp_velocity_limit=80, tcp_acceleration_limit=160)

    cases: list[tuple[str, list]] = [
        (
            "write_before_motion",
            [
                io_write("io_write_case_start", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(forward_pose, fast_back_and_forth),
                ptp(start_pose, fast_back_and_forth),
            ],
        ),
        (
            "write_between_motions",
            [
                ptp(forward_pose, fast_back_and_forth),
                io_write("io_write_case_middle", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(start_pose, fast_back_and_forth),
            ],
        ),
        (
            "write_after_motion",
            [
                ptp(forward_pose, fast_back_and_forth),
                ptp(start_pose, fast_back_and_forth),
                io_write("io_write_case_end", True, origin=api.models.IOOrigin.BUS_IO),
            ],
        ),
        (
            "multiple_writes",
            [
                io_write("io_write_case_many", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(forward_pose, fast_back_and_forth),
                io_write("io_write_case_many", False, origin=api.models.IOOrigin.BUS_IO),
                ptp(start_pose, fast_back_and_forth),
                io_write("io_write_case_many", True, origin=api.models.IOOrigin.BUS_IO),
            ],
        ),
        (
            "write_only",
            [
                io_write("io_write_case_write_only_a", True, origin=api.models.IOOrigin.BUS_IO),
                io_write("io_write_case_write_only_b", True, origin=api.models.IOOrigin.BUS_IO),
                io_write("io_write_case_write_only_a", False, origin=api.models.IOOrigin.BUS_IO),
            ],
        ),
        (
            "async_only",
            [
                async_action(
                    "examples.io_write_action_list_repro.short_async_marker",
                    action_id="async_only",
                    label="async_only",
                )
            ],
        ),
        (
            "async_await_only",
            [
                async_action(
                    "examples.io_write_action_list_repro.short_async_marker",
                    action_id="async_await_only",
                    label="async_await_only",
                ),
                await_action("async_await_only", timeout=2.0),
            ],
        ),
        (
            "async_before_write",
            [
                async_action(
                    "examples.io_write_action_list_repro.short_async_marker",
                    action_id="async_before_write",
                    label="before_write",
                ),
                io_write("io_write_case_async_a", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(forward_pose, fast_back_and_forth),
                ptp(start_pose, fast_back_and_forth),
            ],
        ),
        (
            "write_async_write",
            [
                io_write("io_write_case_async_a", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(forward_pose, fast_back_and_forth),
                async_action(
                    "examples.io_write_action_list_repro.short_async_marker",
                    action_id="write_async_write",
                    label="between_writes",
                ),
                io_write("io_write_case_async_b", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(start_pose, fast_back_and_forth),
            ],
        ),
        (
            "async_with_await_and_write",
            [
                io_write("io_write_case_async_a", True, origin=api.models.IOOrigin.BUS_IO),
                async_action(
                    "examples.io_write_action_list_repro.short_async_marker",
                    action_id="async_with_await",
                    label="awaited",
                ),
                await_action("async_with_await", timeout=2.0),
                ptp(forward_pose, fast_back_and_forth),
                io_write("io_write_case_async_b", True, origin=api.models.IOOrigin.BUS_IO),
                ptp(start_pose, fast_back_and_forth),
            ],
        ),
        (
            "async_and_write_only",
            [
                async_action(
                    "examples.io_write_action_list_repro.short_async_marker",
                    action_id="async_write_only",
                    label="write_only_mix",
                ),
                io_write("io_write_case_async_a", True, origin=api.models.IOOrigin.BUS_IO),
                await_action("async_write_only", timeout=2.0),
                io_write("io_write_case_async_b", True, origin=api.models.IOOrigin.BUS_IO),
            ],
        ),
    ]

    for case_name, actions in cases:
        await reset_signals()
        print(f"Running case: {case_name}")
        try:
            await motion_group.plan_and_execute(actions, tcp)
        except Exception as exc:
            raise RuntimeError(f"Case '{case_name}' failed") from exc

        signal_values = await get_bus_io_value(SIGNALS)
        log_case(case_name, signal_values)


if __name__ == "__main__":
    run_program(main)
