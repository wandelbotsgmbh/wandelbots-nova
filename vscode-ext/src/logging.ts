...
const channel = vscode.window.createOutputChannel('Wandelbots NOVA', { log: true });
...
function write(level: Level, scope: string | undefined, parts: unknown[]) {
  ...
  switch (level) {
    case 'trace': channel.trace(msg); break;
    case 'debug': channel.debug(msg); break;
    case 'info':  channel.info(msg);  break;
    case 'warn':  channel.warn(msg);  break;
    case 'error': channel.error(msg); break;
  }

  // Mirror to console only when the global log level is Debug or lower
  if (vscode.env.logLevel <= vscode.LogLevel.Debug) {
    const c = level === 'error' ? console.error
