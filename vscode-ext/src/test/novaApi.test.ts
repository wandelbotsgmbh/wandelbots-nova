...
suite('NovaApi Tests', () => {
  ...
  suite('NovaApi class instantiation', () => {
    test('should create instance correctly', () => {
      ...
      assert.strictEqual(typeof api.connect, 'function', 'connect method should exist')
      assert.strictEqual(typeof api.getControllers, 'function', 'getControllers method should exist')
      assert.strictEqual(typeof api.getMotionGroups, 'function', 'getMotionGroups method should exist')
      assert.strictEqual(typeof api.getTcps, 'function', 'getTcps method should exist')
      assert.strictEqual(typeof api.getRobotPose, 'function', 'getRobotPose method should exist')
      assert.strictEqual(typeof api.getActiveTcp, 'function', 'getActiveTcp method should exist')
      assert.strictEqual(typeof api.dispose, 'function', 'dispose method should exist')
    })

    test('should have initial null state', () => {
      const api = new NovaApi()
      assert.strictEqual((api as any).client, null, 'client should be initially null')
      assert.strictEqual((api as any).config, null, 'config should be initially null')
    })
  })
  ...
  suite('NovaApi configuration interface', () => {
    test('should accept valid NovaConfig structure', () => {
      assert.strictEqual(typeof config.apiUrl, 'string', 'apiUrl should be string')
      assert.strictEqual(typeof config.accessToken, 'string', 'accessToken should be string')
      assert.strictEqual(typeof config.cellId, 'string', 'cellId should be string')
      assert.ok(config.apiUrl.length > 0, 'apiUrl should not be empty')
      assert.ok(config.accessToken.length > 0, 'accessToken should not be empty')
    ...
    test('should validate NovaConfig required properties', () => {
      ...
      assert.ok('accessToken' in config, 'config should have accessToken property')
  ...
  suite('NovaApi method signatures', () => {
    test('should have correct connect method signature', () => {
      ...
      assert.strictEqual(typeof novaApi.connect, 'function', 'connect method should exist')
      assert.strictEqual(novaApi.connect.length, 1, 'connect method should take 1 parameter')
    })

    test('should have correct getControllers method signature', () => {
      assert.strictEqual(typeof novaApi.getControllers, 'function', 'getControllers method should exist')
      assert.strictEqual(novaApi.getControllers.length, 0, 'getControllers method should take 0 parameters')
    })

    test('should have correct getMotionGroups method signature', () => {
      assert.strictEqual(typeof novaApi.getMotionGroups, 'function', 'getMotionGroups method should exist')
      assert.strictEqual(novaApi.getMotionGroups.length, 1, 'getMotionGroups method should take 1 parameter')
    })

    test('should have correct getTcps method signature', () => {
      assert.strictEqual(typeof novaApi.getTcps, 'function', 'getTcps method should exist')
      assert.strictEqual(novaApi.getTcps.length, 1, 'getTcps method should take 1 parameter')
    })

    test('should have correct getRobotPose method signature', () => {
      assert.strictEqual(typeof novaApi.getRobotPose, 'function', 'getRobotPose method should exist')
      assert.strictEqual(novaApi.getRobotPose.length, 2, 'getRobotPose method should take 2 parameters')
    })

    test('should have correct getActiveTcp method signature', () => {
      assert.strictEqual(typeof novaApi.getActiveTcp, 'function', 'getActiveTcp method should exist')
      assert.strictEqual(novaApi.getActiveTcp.length, 1, 'getActiveTcp method should take 1 parameter')
    })
  })
  ...
  suite('NovaApi error message consistency', () => {
    test('should have consistent error message for unconnected state', async () => {
      ...
      try {
        ...
        assert.strictEqual((error as Error).message, expectedError, 'Error message should match expected')
      }
