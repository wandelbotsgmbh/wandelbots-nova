    ...
    def start(
        ...
        async def _on_state_change():
            async def publish_program_run_to_nats(program_run: ProgramRun):
                ...
                data["start_time"] = (
                    timestamp.datetime_to_rfc3339(program_run.start_time)
                    if program_run.start_time
                    else None
                )
                data["end_time"] = (
                    timestamp.datetime_to_rfc3339(program_run.end_time)
                    if program_run.end_time
                    else None
                )
