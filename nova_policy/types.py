"""Data types for the nova_policy package."""
...
class PolicyResult:
    """Internal base class for policy responses.

    Used inside the executor to dispatch on response type.
    Policy implementations do NOT need to use these types —
    they just return dicts/JSON. The PolicyClient translates.
    """


class PolicyDone(PolicyResult):
    """Internal: episode is done, trigger on_reset."""


class PolicyWaiting(PolicyResult):
    """Internal: policy not ready, hold position."""


class PolicyResponse(pydantic.BaseModel):
    """Serializable response from a policy server.

    This is the wire format — a single model that covers all response types.
    Works across languages (JSON-compatible). Use this on the policy side.
    ...
    Examples:
        ...
        # Episode done:
        PolicyResponse(done=True)

        # Not ready:
        PolicyResponse(waiting=True)

    JSON schema is auto-generated via pydantic and available at runtime.
    """

    joints: dict[str, list[list[float]]] | None = None
    """Motion group id → sequence of joint targets (radians). Structured mode."""
    ...
    E.g. {"left_joint_1.pos": 0.1, "left_gripper.pos": 50.0, ...}
    Used when the policy outputs named features instead of structured motion groups."""

    dt_ms: float = 0.0
    """Time spacing between steps in milliseconds."""

    done: bool = False
    """True if this episode is finished."""

    waiting: bool = False
    """True if the policy is not ready yet."""
...
class PolicyRunnerConfig:
    ...
    Defaults match what was used during training in the lerobot pipeline.
    Most users should NOT change these — the PID parameters must be consistent
    between recording and inference.
    """
