"""Policy Robot Controller — uses PolicyExecutor for lifecycle management."""
...
from nova_policy import PolicyExecutor, WebSocketPolicyClient
...
app = FastAPI(
    ...
    version="0.2.0",
    description="Manages robot lifecycle for policy execution via PID jogging.",
    root_path=BASE_PATH,
    docs_url="/",
)
...
class StartRequest(BaseModel):
    policy_url: str = Field(
        default="ws://app-mock-policy-service:8080/cell/mock-policy-service/predict",
        description="WebSocket URL of the policy service",
    )
...
async def start(req: StartRequest = StartRequest()):
    """Start the executor: connect to robots, open jogging, listen for policy.

    The executor stays alive and reacts to policy starts/stops automatically.
    Between episodes, the on_reset callback moves robots to home.
    """
    ...
    _executor = PolicyExecutor(
        motion_groups=mgs,
        policy=WebSocketPolicyClient(req.policy_url),
        on_reset=on_reset,
