...
from nova.runtime.runner import ExecutionContext, Program, ProgramRunner, ProgramRunState, ProgramType
...
def test_successful_state_transition(sample_program, mock_robot_cell):
    ...
    runner = MockProgramRunner(sample_program, {}, execution_delay=0.05, robot_cell_override=mock_robot_cell)
...
def test_failed_state_transition(sample_program, mock_robot_cell):
    ...
    runner = MockProgramRunner(sample_program, {}, should_fail=True, execution_delay=0.05, robot_cell_override=mock_robot_cell)
...
def test_not_plannable_state_transition(sample_program, mock_robot_cell):
    ...
    runner = MockProgramRunner(sample_program, {}, should_not_plannable=True, execution_delay=0.05, robot_cell_override=mock_robot_cell)
...
def test_stop_during_execution(sample_program, mock_robot_cell):
    ...
    runner = MockProgramRunner(sample_program, {}, execution_delay=1.0, robot_cell_override=mock_robot_cell)
    ...
    if runner.is_running():
        ...
        runner.join()
        assert runner.state in [ProgramRunState.COMPLETED, ProgramRunState.FAILED, ProgramRunState.STOPPED]
...
def test_execution_timing_recorded(sample_program, mock_robot_cell):
    ...
    runner = MockProgramRunner(sample_program, {}, execution_delay=0.1, robot_cell_override=mock_robot_cell)
