...
class TestCycle:
    ...
    async def test_start(self, mock_cell):
        ...
        # Mock NATS publish to avoid actual event dispatch
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()) as mock_publish:
            ...
            assert publish_kwargs["subject"] == "nova.v2.cells.test-cell-1.cycle"

            # Parse and verify the event data
            event = CycleStartedEvent.model_validate_json(publish_kwargs["payload"])
    ...
    async def test_start_already_running(self, mock_cell):
        ...
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()):
            await cycle.start()
    ...
    async def test_finish(self, mock_cell):
        ...
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()) as mock_publish:
            ...
            # Parse and verify the event data
            event = CycleFinishedEvent.model_validate_json(publish_kwargs["payload"])
            assert event.cycle_id == cycle.cycle_id
            assert event.cell == "test-cell-1"
    ...
    async def test_fail(self, mock_cell):
        ...
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()) as mock_publish:
            ...
            # Parse and verify the event data
            event = CycleFailedEvent.model_validate_json(publish_kwargs["payload"])
            assert event.cycle_id == cycle.cycle_id
            assert event.cell == "test-cell-1"
            assert isinstance(event.timestamp, datetime)
    ...
    async def test_context_manager_success(self, mock_cell):
        ...
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()) as mock_publish:
            ...
            start_call, finish_call = mock_publish.await_args_list
            assert start_call.kwargs["subject"] == "nova.v2.cells.test-cell-1.cycle"
            assert finish_call.kwargs["subject"] == "nova.v2.cells.test-cell-1.cycle"

            start_event = CycleStartedEvent.model_validate_json(start_call.kwargs["payload"])
            finish_event = CycleFinishedEvent.model_validate_json(finish_call.kwargs["payload"])

            assert start_event.cycle_id == cycle.cycle_id
            assert finish_event.cycle_id == cycle.cycle_id
    ...
    async def test_context_manager_exception(self, mock_cell):
        ...
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()) as mock_publish:
            ...
            start_call, fail_call = mock_publish.await_args_list
            assert start_call.kwargs["subject"] == "nova.v2.cells.test-cell-1.cycle"
            start_event = CycleStartedEvent.model_validate_json(start_call.kwargs["payload"])
            assert start_event.cell == "test-cell-1"

            # Check second event (fail)
            assert fail_call.kwargs["subject"] == "nova.v2.cells.test-cell-1.cycle"
            fail_event = CycleFailedEvent.model_validate_json(fail_call.kwargs["payload"])
    ...
    async def test_cycle_with_extra(self, mock_cell):
        ...
        with patch.object(mock_cell.nats, "publish", new=AsyncMock()) as mock_publish:
            async with Cycle(mock_cell, extra=extra):
                pass
